...
try
{
  ...
  echo "<h1>Client List Filter - " . $ts3_ServerInstance . "</h1>\n";
  ...
  /* set some example filters */
  $filter = array(
    "client_nickname" => "[^[:alnum:]]",
    "client_type" => TeamSpeak3::CLIENT_TYPE_REGULAR,
  );

  /* display filter info */
  echo "<b class=\"pre\">Nickname Filter:</b> [^[:alnum:]]<br /><br />\n";
  ...
  echo "<tr>\n" .
       ...
       "  <th>Version</th>\n" .
       "</tr>\n";
  foreach($ts3_VirtualServer->clientList($filter) as $client)
  {
    echo "<tr>\n" .
         "  <td>" . $client->getId() . "</td>\n" .
