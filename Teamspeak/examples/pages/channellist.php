...
try
{
  ...
  echo "<h1>Channel List - " . $ts3_ServerInstance . "</h1>\n";
  ...
  foreach($ts3_VirtualServer->channelList() as $channel)
  {
    echo "<tr>\n" .
         "  <td>" . $channel->getId() . "</td>\n" .
         "  <td><a href=\"?page=channelinfo&amp;server=" . $ts3_VirtualServer->getId() . "&amp;channel=" . $channel->getId() . "\">" . htmlspecialchars($channel) . "</a></td>\n" .
         "  <td><img src=\"../images/viewer/" . $channel->getIcon() . ".png\" alt=\"\" /></td>\n" .
         "  <td>" . $channel["total_clients"] . " / " . ($channel["channel_maxclients"] == -1 ? "Unlimited" : $channel["channel_maxclients"]) . "</td>\n" .
         "  <td>" . $channel->getLevel() . "</td>\n" .
         "  <td>" . $channel->getPathway() . "</td>\n" .
         "</tr>\n";
  }
  echo "</table>\n";
