 ...
 *
 * $Id: Server.php 3/19/2011 10:09:58 scp@orilla $
 *
...
class TeamSpeak3_Node_Server extends TeamSpeak3_Node_Abstract
{
   ...
   *
   * @todo   [SERVER LIMITATION] return real file transfer host address instead of ServerQuery host address
  ...
  public function transferInitUpload($clientftfid, $cid, $name, $size, $cpw = "", $overwrite = FALSE, $resume = FALSE)
  {
    ...
    if(!array_key_exists("host", $upload)) $upload["host"] = $this->getParent()->getAdapterHost();
  ...
  public function transferInitDownload($clientftfid, $cid, $name, $cpw = "", $seekpos = 0)
  {
    ...
    if(!array_key_exists("host", $download)) $download["host"] = $this->getParent()->getAdapterHost();
  ...
  protected static function sortGroupList(TeamSpeak3_Node_Abstract $a, TeamSpeak3_Node_Abstract $b)
  {
    ...
    if($a->getProperty("sortid", 0) != $b->getProperty("sortid", 0))
    {
